...
export default function EditStrategyPage() {
  ...
  const params: StrategyParams = form.params ?? DEFAULT_PARAMS;
  const showWeight = params.evalMode === 'SCORE';
    ...
    <div style={{ minHeight: '100vh', background: '#030712', padding: '24px' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
              ...
              {(['ALL','ANY','SCORE'] as EvalMode[]).map(m => (
                <button key={m} onClick={() => setParam('evalMode', m)}
                  style={{ padding: '6px 14px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold',
                    background: params.evalMode === m ? '#EAB308' : '#374151',
                    color: params.evalMode === m ? '#111827' : '#9CA3AF' }}>
                  {evalLabels[m]}
                </button>
              ))}
            </div>
            {showWeight && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <input style={{ ...inp, width: '72px' }} type="number" min={0} max={100}
                  value={params.minScore} onChange={e => setParam('minScore', +e.target.value)} />
              </div>
            )}
          </div>
          <RuleBuilder title="롱 진입 조건" rules={params.longEntryRules ?? []} showWeight={showWeight} onChange={r => setParam('longEntryRules', r)} />
        ...
        <div style={{ ...sec, border: '1px solid #1E3A5F' }}>
          ...
          <RuleBuilder title="차단 조건" excludeRuleTypes={["TEST_FORCE_ENTRY_ONCE"]} rules={params.blockRules ?? []} showWeight={false} onChange={r => setParam('blockRules', r)} />
        </div>
